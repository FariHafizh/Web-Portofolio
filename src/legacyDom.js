    ...
    slider.addEventListener('transitionend', () => {
      ...
      if (slides[currentIndex] && slides[currentIndex].classList.contains('clone')) {
        if (currentIndex === 0) {
          ...
          slider.style.transform = `translateX(${ -currentIndex * 100 }%)`;
          // descriptions updated by moveTo normally; ensure in-sync
          const originalsCount = originalSlides.length;
      ...
      if (announcer) {
        ...
        announcer.textContent = desc && desc.querySelector('h3') ? desc.querySelector('h3').textContent : `Slide ${mapped + 1}`;
      }
    });
